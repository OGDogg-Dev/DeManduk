...
<form method="POST" action="{{ $action }}" enctype="multipart/form-data" class="space-y-6">
    ...
    <div class="space-y-2">
        ...
        <input type="file" name="image" accept="image/*" class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200" {{ $project ? '' : 'required' }}>
        @error('image')
        ...
        @if ($project && $project->image_path)
            <div class="space-y-2 rounded-xl border border-slate-200 bg-slate-50 p-3">
                ...
                <img src="{{ \App\Support\Media::url($project->image_path) }}" alt="{{ $project->title }}" class="h-32 w-full rounded-lg object-cover">
