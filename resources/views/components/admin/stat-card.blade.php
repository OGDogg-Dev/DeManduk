...
<article class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
    <div class="flex items-center justify-between">
        <div>
            <p class="text-xs font-semibold uppercase tracking-[0.35em] text-slate-500">{{ $label }}</p>
            <p class="mt-3 text-3xl font-bold text-slate-900">{{ $value }}</p>
        </div>
        <span class="inline-flex h-11 w-11 items-center justify-center rounded-xl bg-indigo-50 text-indigo-500">
    ...
    @if ($trend)
        <p class="mt-3 inline-flex items-center gap-2 text-xs font-semibold {{ $trend > 0 ? 'text-emerald-600' : 'text-amber-600' }}">
            ...
            <span class="font-medium text-slate-400">{{ $trendLabel ?? 'Perubahan minggu ini' }}</span>
        </p>
    @endif
</article>
